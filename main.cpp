 ...
 * 4 bits an entry,  iterate the 64 bits row from left to right
 * +------------------------+
 * |     2     8   128     4|
 * |     8    32    64   256|
 * |     2     4    32   128|
...
public:
	...
	void place(int i, int r) {
	    raw =
	            (raw & ~(0xffffULL << (i << 4))) | // Erase the target row
	            (uint64_t(r & 0xffff) << (i << 4)); // Move the input r into the target row
	}
	/**
	 * get a 4-bit tile
...
public:
	...
	feature(const feature& f) = delete; // Forbid the copy constructor
	feature& operator =(const feature& f) = delete; // Forbid the assign copy
...
public:
	 ...
	 * This method is used to show the evaluated value of a board
	 * The estimate function is the one I need to implement
	 */
...
public:
	...
	void load(const std::string& path) {
		...
		if (in.is_open()) {
			...
			// The size must be matched with the feats size
